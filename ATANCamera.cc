...
void ATANCamera::RefreshParams() 
{
  ...
  mdMaxR = 1.7 * mdLargestRadius; // (pretty arbitrary)
...
void ATANCamera::computeocvCameraMatrix()
{
  mocvCameraMatrix.release();
  mocvCameraMatrix.create(3,3,CV_32F);
  *(mocvCameraMatrix.ptr<float>(0,0)) = mvFocal[0];
  *(mocvCameraMatrix.ptr<float>(0,1)) = 0;
  *(mocvCameraMatrix.ptr<float>(0,2)) = mvCenter[0];//-0.5;
  *(mocvCameraMatrix.ptr<float>(1,0)) = 0;
  *(mocvCameraMatrix.ptr<float>(1,1)) = mvFocal[1];
  *(mocvCameraMatrix.ptr<float>(1,2)) = mvCenter[1];//-0.5;
  *(mocvCameraMatrix.ptr<float>(2,0)) = 0;
  *(mocvCameraMatrix.ptr<float>(2,1)) = 0;
  *(mocvCameraMatrix.ptr<float>(2,2)) = 1;
}
