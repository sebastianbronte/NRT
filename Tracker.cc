 ...
 * @param mm map maker
 */
Tracker::Tracker(ImageRef irVideoSize, const ATANCamera &c, Map &m, int levels) :
  mCurrentKF(levels),
  mMap(m),
  mCamera(c),
  ...
  mirSize(irVideoSize),
  mFirstKF(levels),
  mPreviousFrameKF(levels),
  mnFrame(0),
  areCoeffsLoadedFromDisk(false),
  mdRMSError(0),
  mStateEstimator(mCamera,mse3CamFromWorld),
  mbMapGrowing(true)
{
  ...
  iniframe1=GV3::get<int>("iniframe1",-1);
  iniframe2=GV3::get<int>("iniframe2",-1);
...
Tracker::~Tracker()
{
  delete mpSBIThisFrame;
  delete mpSBILastFrame;
  delete mCurrentKF.pSBI;
}
...
void Tracker::Reset()
{
  ...
  // As this version does not have an explicit MapMaker, we do not need to tell
  // it to reset itself
#ifndef WIN32
  usleep(10);
#else
...
void Tracker::TrackFrame(Image<CVD::byte> &imFrame, bool bDraw)
{
  ...
  if(mMap.IsGood())
  {
    ...
    else  // what if there is a map, but tracking has been lost?
    {
      ...
      cout << "LOST tracking. Attempting recovery" << endl;
      if(AttemptRecovery())
      {
        TrackMap();
...
void Tracker::TrackMap()
{
  ...
  for(unsigned int i=0; i<mMap.vpPoints.size(); i++)
  {
    ...
    if(!p.pTData) p.pTData = new NRTrackerData(&p);
  ...
  if(bTryCoarse && avPVS[mCurrentKF.MAXLEVELS-1].size() + avPVS[mCurrentKF.MAXLEVELS-2].size() > *gvnCoarseMin )
  {
    ...
    // NRTrackerData's corresponding to coarse map points! This depends on how many
    // there are in different pyramid levels compared to CoarseMin and CoarseMax.

    if(avPVS[mCurrentKF.MAXLEVELS-1].size() <= nCoarseMax) 
    {
      // Fewer than CoarseMax in MAXLEVELS-1? then take all of them, and remove
      // them from the PVS list.
      vNextToSearch = avPVS[mCurrentKF.MAXLEVELS-1];
      avPVS[mCurrentKF.MAXLEVELS-1].clear();
    }
    else
    {
      // ..otherwise choose nCoarseMax at random, again removing from the PVS
      // list.
  ...
  int nFineRange = nCoarseRange/2; //10 // Pixel search range for the fine stage.
  ...
  if(mbDraw)
  {
    ...
    //introduce reference coordinate rendering
    glBegin(GL_LINES);
    glLineWidth(3);
    glColor3f(1,0,0);
...
/**
 * Find points in the image. Uses the PatchFiner struct stored in NRTrackerData
 * @param vTD tracker data
 * @param nRange search range
 * @param nSubPixIts number of sub-pixel iterations required
 * @return number of points found
 */
int Tracker::SearchForPoints(vector<NRTrackerData*> &vTD, int nRange, int nSubPixIts)
{
  int nFound = 0;
 ...
 * @return 
 */
Vector<6> Tracker::CalcPoseUpdate(vector<NRTrackerData*> &vTD, double dOverrideSigma, bool bMarkOutliers)
{
  return mStateEstimator.CalcPoseUpdate(vTD,dOverrideSigma,bMarkOutliers);
}
...
void Tracker::UpdateMotionModel()
{
  ...
  static gvar3<double> gvdMotionModelWeight("Tracker.MotionModelWeight", 0.9, SILENT);

  mv6CameraVelocity = *gvdMotionModelWeight * (0.5 * v6Motion + 0.5 * v6OldVel);
...
void Tracker::AssessTrackingQuality()
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(dLargeFracFound < *gvdQualityLost)
        mTrackingQuality = BAD;
      else
  ...
  if(mTrackingQuality == DODGY)
  {
    ...
    // MapMaker is not released in this version. We directly transition from DODGY to BAD.
...
double Tracker::computeProjError(vector<NRTrackerData*> &vTD)
{
    double meanProjectionError=0.0;
    ...
    for(unsigned int f=0; f<vTD.size(); f++)
    {
      ...
      if(!TD->bFound)
        continue;
      meanProjectionError += sqrt(TD->v2Error_CovScaled[0]*TD->v2Error_CovScaled[0]+TD->v2Error_CovScaled[1]*TD->v2Error_CovScaled[1]);
    }

    return meanProjectionError/vTD.size();
}

double Tracker::computeProjError_RMS(vector<NRTrackerData*> &vTD)
{
    //change this algorithm to implement RMS metric.
    double meanProjectionError=0.0;
    double meanProjection=0.0;

    if(vTD.size() == 0)
    {
      cout << "CAUTION!!!!!! NO POINTS THERE!!!!!" << endl;
      return -1;
    }
    ...
    for(unsigned int f=0; f<vTD.size(); f++)
    {
      ...
      if(!TD->bFound)
        continue;
      meanProjectionError += TD->v2Error_CovScaled[0]*TD->v2Error_CovScaled[0]+TD->v2Error_CovScaled[1]*TD->v2Error_CovScaled[1];
      meanProjection += TD->v2Found[0] * TD->v2Found[0] + TD->v2Found[1]*TD->v2Found[1];
    }

    return sqrt(meanProjectionError/meanProjection);
}

double Tracker::computeBestKFProjError_RMS(vector<NRTrackerData*> &vTD)
{
    double meanKFProjectionError=1e20;
    double meanKFProjection=1e20;
    unsigned int bestkf = mMap.vpKeyFrames.size()+1;
    for(unsigned int kf=0;kf<mMap.vpKeyFrames.size();kf++)
    {
      KeyFrame* KF = mMap.vpKeyFrames[kf];
      //change this algorithm to implement RMS metric.
      double meanProjectionError=0.0;
      double meanProjection=0.0;
          ...
          it++)
      {
        if(!(*it)->bFound)
          continue;
        if(KF->mMeasurements.find(& ((*it)->Point)) == KF->mMeasurements.end())
          continue;
        Vector<2> v2Found = (*it)->v2Found;
        Vector<2> v2Error_CovScaled = (*it)->v2Error_CovScaled;
        meanProjectionError += v2Error_CovScaled[0]*v2Error_CovScaled[0] + v2Error_CovScaled[1]*v2Error_CovScaled[1];
        meanProjection += v2Found[0]*v2Found[0] + v2Found[1]*v2Found[1];
      }

      if(meanKFProjectionError>meanProjectionError && meanProjection != 0)
      {
        meanKFProjectionError = meanProjectionError;
        meanKFProjection = meanProjection;
        bestkf = kf;
      }
    }

    if(bestkf==(mMap.vpKeyFrames.size()+1))
    {
      return 1e20;
    }
    return sqrt(meanKFProjectionError/meanKFProjection);
}
