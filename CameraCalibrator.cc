...
int main(int argc, char *argv[])
{
  ...
  string opt1 = GV3::get<string>("i","cam");
  if(!opt1.compare("rec"))
  {
  ...
  {
      cout << "  No valid input option has given. exiting" << endl;
      return 0;
  }
  ...
  try
    {
      CameraCalibrator c(mpVideoSource);
      c.Run();
    }
...
CameraCalibrator::CameraCalibrator(VideoSource * mpVideoSource)
  :mVideoSource(mpVideoSource),mGLWindow(mVideoSource->Size(), "Camera Calibrator"), mCamera("Camera")
{
  mbDone = false;
