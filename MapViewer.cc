...
/**
 * Draw the current bases shape for the set of bases, computed by
 * Interpolation class
 *
 * @param shape3D
 * @return
 */
void MapViewer::DrawBasesShape(SE3<> se3ModelToWorld,
                               vector<Vector<3> > & shape3D)
{
  SetupModelView();
  SetupFrustum();
  ...
  glPointSize(3);
  glBegin(GL_POINTS);
  mv3BasesMassCenter = Zeros;
  cout << "relative pose model2world " << se3ModelToWorld << endl;
  cout << "relative viewer from world pose " << mse3ViewerFromWorld << endl;
  cout << "shape 3D size " << shape3D.size() << endl;
  for(size_t i=0; i<shape3D.size(); i++)
  {
    Vector<3> v3Pos = se3ModelToWorld * shape3D[i];
    glColor3f(1.0,0.0,1.0);
    if( (v3Pos * v3Pos) < 1000000) // high enough limit
    {
      nForMass++;
      mv3BasesMassCenter += v3Pos;
    }
    glVertex(v3Pos);
  }
  glEnd();
  mv3BasesMassCenter = mv3BasesMassCenter / (0.1 + nForMass);
