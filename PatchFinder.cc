  ...
  : mimTemplate(ImageRef(nPatchSize,nPatchSize))
{
  ...
  int nMaxSSDPerPixel = GV2.GetInt("MaxSSDPerPixel", 500, SILENT);
...
int PatchFinder::CalcSearchLevelAndWarpMatrix(NRMapPoint &p,
                                              SE3<> se3CFromW,
                                              Matrix<2> &m2CamDerivs)
{
  ...
  // Additional note. LEVELS in this context could not successfully changed by
  // MAXLEVELS as there is no KF involved. In addition, this class will not be used
  // when a different number of the predefined levels is used.
...
// This is just a convenience function which caluclates the warp matrix and generates
// the template all in one call.
void PatchFinder::MakeTemplateCoarse(NRMapPoint &p,
                                     SE3<> se3CFromW,
                                     Matrix<2> &m2CamDerivs)
{
...
void PatchFinder::MakeTemplateCoarseCont(NRMapPoint &p)
{
...
void PatchFinder::MakeTemplateCoarseNoWarp(NRMapPoint &p)
{
...
{
  double dConvLimit = GV2.GetDouble("ConvergenceLimit", 0.03, SILENT);
...
// Calculate the Zero-mean SSD of the coarse patch and a target image at a specific
// point.
