...
public:
  ...
  // transformation.
  int CalcSearchLevelAndWarpMatrix(NRMapPoint &p, SE3<> se3CFromW, Matrix<2> &m2CamDerivs);
  ...
  void MakeTemplateCoarseCont(NRMapPoint &p); // If the warping matrix has already been pre-calced, use this.
  void MakeTemplateCoarse(NRMapPoint &p, SE3<> se3CFromW, Matrix<2> &m2CamDerivs); // This also calculates the warp.
  void MakeTemplateCoarseNoWarp(NRMapPoint &p);  // Identity warp: just copies pixels from the source KF.
  void MakeTemplateCoarseNoWarp(KeyFrame &k, int nLevel, CVD::ImageRef irLevelPos); // Identity warp if no NRMapPoint struct exists yet.
...
protected:
  ...
  NRMapPoint *mpLastTemplateMapPoint;  // Which was the last map point this PatchFinder used?
